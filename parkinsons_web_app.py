...
def is_silent(path, threshold=0.01):
    ...
    try:
        ...
        # On error, do not treat as silent to avoid blocking everything
        return False
...
# ---------- BUILD SPOKEN TEXT (translations + fallback English) ----------
def build_spoken_text(probability_pd, mode_text, risk_level, case_type, lang_label):
    ...
    translations = {
        ...
        "Tamil": {
            "low":    f"Lakshanangal kuraindha abaya nilaiyai kaattukinrana. {mode_text} voice il {p} sathavitham Parkinson sathiyamaana saadhiyam.",
            "medium": f"Lakshanangal madhyama abaya nilaiyai kaattukinrana. {mode_text} voice il {p} sathavitham Parkinson saadhiyam.",
            "high":   f"Lakshanangal adhiga abaya nilaiyai kaattukinrana. {mode_text} voice il {p} sathavitham Parkinson saadhiyam. Doctor ai sandhithal nallathu.",
        },
        "Kannada": {
...
if (audio_file is not None or st.session_state.recorded_audio_path) and st.button(
    ...
    try:
        with st.spinner("🎯 Analyzing voice features..."):
            ...
            # Silence check – if no real voice, stop here
            if is_silent(temp_path):
                st.warning("🎤 It seems no voice was recorded. Please record again saying 'AH' clearly for 5 seconds.")
        ...
        # Decide case type for text
        if probability_pd < 0.8:
            case_type = "low"
            st.markdown("---")
            st.success(
                f"✅ **YOU ARE HEALTHY** - {mode_text.upper()} voice analysis: "
            ...
            st.markdown("---")
            st.error(
                f"🚨 **PARKINSON'S DISEASE DETECTED** - {mode_text.upper()} voice analysis: "
            ...
            st.markdown("---")
            st.warning(
                f"⚠️ Mixed result - {mode_text.upper()} voice analysis: "
                f"**{probability_pd*100:.0f}%** Parkinson's probability"
            )
        ...
        # Silent recording already showed a warning; no extra error needed
